...
mp_image1 = cv2.morphologyEx(b_image, cv2.MORPH_OPEN,
morph_rect, iterations = 1)
mp_image2 = cv2.morphologyEx(b_image, cv2.MORPH_OPEN,
morph_cros, iterations = 1)
mp_image3 = cv2.morphologyEx(b_image, cv2.MORPH_OPEN,
morph_elip, iterations = 1)
mp_image4 = cv2.morphologyEx(b_image, cv2.MORPH_OPEN,
morph_rect, iterations = 3)

cv2.imshow('Original image', g_image)
cv2.imshow('Binary image', b_image)
cv2.imshow('Opening image - rect iter 1', mp_image1)
cv2.imshow('Opening image - cross iter 1', mp_image2)
cv2.imshow('Opening image - ellipse iter 1', mp_image3)
cv2.imshow('Opening image - rect iter 4', mp_image4)
